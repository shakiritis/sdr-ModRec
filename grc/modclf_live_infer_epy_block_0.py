...
class blk(gr.sync_block):
    def __init__(self, model_path="models/modclf.onnx", labels_path="models/labels.json",
                 win_len=1024, hop_len=256, print_on_change=True):
    ...
    def work(self, input_items, output_items):
        x = input_items[0]
        y = output_items[0]     # complex passthrough
        ylab = output_items[1]  # float label index

        y[:] = x
        ylab[:] = self.current_pred  # hold last prediction
...
#                        self.add_item_tag(1, self.nitems_written(1), pmt.intern("mod"), pmt.intern(label))
